...
app.get('/tours', (req, res) => {
  ...
  query += whereClause + 'ORDER BY t.max_seats DESC LIMIT ? OFFSET ?';
...
app.put('/tours/:id', (req, res) => {
  ...
  db.query('SELECT status FROM tours WHERE tour_id = ?', [id], (err, results) => {
    ...
    if (currentStatus !== 'Không hoạt động' && currentStatus !== 'Đã kết thúc' && !(currentStatus === 'Hoạt động' && !hasItinerary)) {
      return res.status(403).send('Only tours with status "Không hoạt động" or "Đã kết thúc" or "Hoạt động" with no itinerary can be edited');
    }
